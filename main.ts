...
namespace mcp79410 {
    ...
    export function isOsf(): boolean {
        if ((getReg(REG_WEEKDAY) & 0x10) != 0) return true;
        else return false;
    }
    ...
    export function resetOsf(): void {
        setReg(REG_WEEKDAY, getReg(REG_WEEKDAY) & 0xef)
        start()
    }
    ...
    export function DateTime(year: number, month: number, day: number, weekday: number, hour: number, minute: number, second: number): void {
        ...
        buf[3] = DecToHex(hour);
        buf[4] = DecToHex(weekday) | 0x08;	// External Battery Backup Supply (VBAT) Enable bit
        buf[5] = DecToHex(day);
     ...
     * @param dat data of EEPROM, eg: 0xaa
     */
    //% blockId="setEEPROM" block="set EEPROM address=%ad data=%dat"
    //% weight=60 blockGap=8
    export function setEEPROM(ad: number, dat: number): void {
        let buf = pins.createBuffer(2);
        buf[0] = ad;
        buf[1] = dat;
        pins.i2cWriteBuffer(EEPROM_ADDR, buf);
    }
